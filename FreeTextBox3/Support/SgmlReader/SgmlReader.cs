    ...
    public class SgmlReader : XmlReader {
            ...
            if (this.dtd == null) {
                if (this.syslit == null || this.syslit == "") {
                    if (this.docType != null && this.docType.ToLower() == "html") {
                        Assembly a = typeof(FreeTextBox).Assembly;
                        string name = "FreeTextBoxControls.Support.SgmlReader.Resources.Html.dtd";
						...
						try {
							...
							throw new Exception(e.ToString() + "//" + name + "\\\\");
						}
                    }
                } else { 
                    if (baseUri != null) {
        ...
        void ParseDocType() {
            ...
            if (ch != '>') {
                ...
                if (ch != '[') {
                    string token = this.ScanName(SgmlReader.dtterm);
                    if (token == "PUBLIC") {
                        ch = this.current.SkipWhitespace();
                        if (ch == '\"' || ch == '\'') {
